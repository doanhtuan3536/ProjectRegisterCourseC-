...
class ScoreListCourse
{
public:
	Course_Result* RegistedCoursee = NULL;
	int NumCourse = 0;
	short Sumcredit = 0; float AverScore = 0; float AverGpa;
};
class semester {
public:
	string SchoolYear = "0";
...
private:
	string CLASS;
	ScoreListCourse RegisterCourse;
public:
...
public:
	...
	void Import_ScoreStudent(semester& a);
	void view_ScoreClass(semester& a);
	void Edit_Profile();
	void update_ScoreStudent(semester& a);
	void change_day(semester& a, date& currentday);
};
